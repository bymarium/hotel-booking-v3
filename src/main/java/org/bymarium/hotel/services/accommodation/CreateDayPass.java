...
public class CreateDayPass implements ICommandParameterized<DayPass, Accommodation> {
  ...
  public DayPass execute(Accommodation accommodation) {
    ...
    if (accommodation instanceof DayPass) {
      dayPass = new DayPass(accommodation.getName(), accommodation.getRate(), accommodation.getCity(), accommodation.getDescription(), accommodation.getType(), accommodation.getServices(), ((DayPass) accommodation).getPersonPrice());
    }
